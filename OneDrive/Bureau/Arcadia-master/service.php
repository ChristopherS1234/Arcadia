...
<body>
    ...
    <?php if (!empty($contenus)): ?>
        <?php foreach ($contenus as $contenu): ?>
            <section class="summary-3 d-flex justify-content-center align-items-center pt-5">

                <div class="single-image">
                    <?php
                    require __DIR__ . '/admin-space/config.php';;
                    $stmt = $pdo->query("SELECT * FROM image ORDER BY image_id DESC LIMIT 1");
    ...
    <?php if (!empty($service)): ?>
        <?php foreach ($service as $contenu): ?>
            <section class="summary-2 justify-content-center align-items-center pt-5">
                <div class="single-image">
                    <?php
                    require __DIR__ . '/admin-space/config.php';
                    $stmt = $pdo->query("SELECT * FROM image_sec ORDER BY image_id DESC LIMIT 1");
                    $image = $stmt->fetch(PDO::FETCH_ASSOC);
                ...
                <div class="container my-5 py-5">
                    <div class="row">
                        <div class="col-md 6">
                            ...
                            <h3 class="text-c">
                                <?= htmlspecialchars($contenu['description']) ?></h3>
                        </div>
                    </div>
                </div>
            </section>
    ...
    <?php if (!empty($serviceLast)): ?>
        <?php foreach ($serviceLast as $contenu): ?>
            <section class="summary-4 d-flex justify-content-center align-items-center pt-5">
                ...
                <div class="container my-5 py-5">
                    <div class="row">
                        <div class="col-md 6">
                            ...
                            <h4 class="text-c">
                                <?= htmlspecialchars($contenu['description']) ?></h4>
                        </div>
                    </div>
                </div>
                ...
                <div class="single-image">
                    <?php
                    require __DIR__ . '/admin-space/config.php';
                    $stmt = $pdo->query("SELECT * FROM image_third ORDER BY image_id DESC LIMIT 1");
                    $image = $stmt->fetch(PDO::FETCH_ASSOC);

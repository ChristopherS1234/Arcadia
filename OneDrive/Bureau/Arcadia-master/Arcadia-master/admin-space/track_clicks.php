    ...
    if (isset($_POST['page_name']) && isset($_POST['element_name'])) {
        ...
        try {
            ...
            if ($result) {
                ...
                $stmt = $pdo->prepare("UPDATE click_stats SET click_count = click_count + 1 WHERE id = ?");
                $stmt->execute([$result['id']]);
            } else {
                // Créer une nouvelle entrée
                $stmt = $pdo->prepare("INSERT INTO click_stats (page_name, element_name, click_count) VALUES (?, ?, 1)");
                $stmt->execute([$page_name, $element_name]);
            }
